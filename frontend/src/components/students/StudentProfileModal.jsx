    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-4xl max-h-[85vh] overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl flex flex-col justify-between text-slate-700 dark:text-slate-200 z-10"
        >
          {/* Close button */}
          <button 
            onClick={onClose}
            className="absolute top-6 right-6 p-2 rounded-xl bg-slate-100 dark:bg-slate-800/60 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-black text-slate-950 dark:text-white tracking-tight">{student.name}</h2>
                <p className="text-xs font-black text-indigo-500 dark:text-indigo-400/80 uppercase tracking-widest mt-1 flex items-center gap-1.5">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <span className="px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border bg-indigo-50 text-indigo-700 border-indigo-200/60 dark:bg-indigo-950/40 dark:text-indigo-400 dark:border-indigo-900/40">
                ⭐ Dean's List Honor
              </span>
              <button 
          ...
          {/* Tabs row */}
          <div className="flex gap-2 border-b border-slate-100 dark:border-slate-800/60 py-3 mb-6 overflow-x-auto scrollbar-none">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                  activeTab === tab 
                    ...
                    : 'bg-slate-100 dark:bg-slate-800/40 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'
                }`}
              >
                {tab}
              </button>
            ...
            <div className="flex-1">
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="p-6 rounded-3xl bg-slate-50/50 dark:bg-slate-950/40 border border-slate-200/60 dark:border-slate-800">
                      <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
                        <ShieldCheck className="h-4.5 w-4.5 text-indigo-500 dark:text-indigo-400" /> Dossier Details
                      </h3>
                    ...
                    <div className="p-6 rounded-3xl bg-slate-50/50 dark:bg-slate-950/40 border border-slate-200/60 dark:border-slate-800 flex flex-col justify-between">
                      <div>
                        <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
                          <Award className="h-4.5 w-4.5 text-amber-500 dark:text-amber-400" /> Key Milestones & Achievements
                        </h3>
              ...
              {activeTab === 'marks' && (
                <div className="space-y-4">
                  <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
                    <BookOpen className="h-4.5 w-4.5 text-indigo-500 dark:text-indigo-400" /> Dynamic Subject Progression
                  </h3>
                  <div className="grid gap-4 sm:grid-cols-2">
                    {subjectPerformance.map(s => (
                      <div key={s.code || s.subject} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-950/40 border border-slate-200/60 dark:border-slate-800">
                        <div>
                          <p className="text-xs font-black text-slate-800 dark:text-white">{s.subject}</p>
                          <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-0.5">{s.code || 'N/A'}</p>
                          <span className={`inline-block text-[9px] font-black uppercase px-2 py-0.5 rounded-full mt-2.5 border ${
                            s.percentage >= 75 ? 'bg-emerald-50 text-emerald-700 border-emerald-200/60 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-900/40' : 'bg-red-50 text-red-700 border-red-200/60 dark:bg-red-950/40 dark:text-red-400 dark:border-red-900/40'
                          }`}>
                            {s.status}
                          </span>
                        </div>
                        <div className="text-right">
                          <p className="text-base font-black text-indigo-600 dark:text-indigo-400">{s.percentage}%</p>
                          <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase mt-0.5">Average Score</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {activeTab === 'attendance' && (
                <div className="space-y-4">
                  <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
                    <Calendar className="h-4.5 w-4.5 text-indigo-500 dark:text-indigo-400" /> Syllabus Attendance Rate
                  </h3>
                  <div className="space-y-3">
                    {subjectPerformance.map(s => {
                      ...
                      return (
                        <div key={s.code || s.subject} className="p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-950/40 border border-slate-200/60 dark:border-slate-800 space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            <span className="font-black text-slate-800 dark:text-white">{s.subject} ({s.code})</span>
                            <span className={`font-black ${rate >= 75 ? 'text-emerald-500' : 'text-red-500'}`}>{rate}%</span>
                          </div>
              ...
              {activeTab === 'trends' && (
                <div className="space-y-4">
                  <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
                    <TrendingUp className="h-4.5 w-4.5 text-indigo-500 dark:text-indigo-400" /> Semester GPA Trajectory
                  </h3>
                  <div className="h-[250px] w-full min-h-[250px] bg-slate-50/50 dark:bg-slate-950/40 p-4 rounded-3xl border border-slate-200/60 dark:border-slate-800">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={semTrends} margin={{ left: -25, right: 10, top: 10, bottom: 0 }}>
                        ...
                        <XAxis dataKey="semester" stroke="#94a3b8" tick={{ fontSize: 10, fontWeight: 'bold' }} />
                        <YAxis domain={[0, 10]} stroke="#94a3b8" tick={{ fontSize: 10, fontWeight: 'bold' }} />
...
function InsightMetric({ title, value, subtitle, icon, color }) {
  const colors = {
    purple: 'bg-purple-50 text-purple-600 border-purple-200/60 dark:bg-purple-950/50 dark:text-purple-400 dark:border-purple-900/40',
    emerald: 'bg-emerald-50 text-emerald-600 border-emerald-200/60 dark:bg-emerald-950/50 dark:text-emerald-400 dark:border-emerald-900/40',
    blue: 'bg-blue-50 text-blue-600 border-blue-200/60 dark:bg-blue-950/50 dark:text-blue-400 dark:border-blue-900/40',
    amber: 'bg-amber-50 text-amber-600 border-amber-200/60 dark:bg-amber-950/50 dark:text-amber-400 dark:border-amber-900/40',
  };
 
  return (
    <div className="p-4 rounded-2xl flex items-center gap-3.5 bg-slate-50/50 dark:bg-slate-950/40 border border-slate-200/60 dark:border-slate-800">
      ...
      <div>
        ...
        <p className="text-base font-black text-slate-900 dark:text-white leading-none mt-0.5">{value}</p>
        <p className="text-[8px] font-bold text-slate-500 dark:text-slate-400 mt-1 truncate">{subtitle}</p>
      </div>
    </div>
  );
}
...
function DetailRow({ icon, label, value }) {
  return (
    <div className="flex items-center gap-3">
      <div className="h-7 w-7 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-slate-400">
        {icon}
      </div>
      <div>
        <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase">{label}</p>
        <p className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">{value}</p>
      </div>
    </div>
  );
}
