    ...
    <div className="space-y-10">
      ...
      <div className="border-t border-slate-100 dark:border-slate-800 pt-10">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900">Mark Attendance</h2>
            <p className="mt-1 text-sm text-slate-500">Select subject and date, then mark students present/absent.</p>
          </div>
              ...
              {filteredStudents.map((st) => (
                <tr key={st.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4">
                    <span className={`rounded-full px-3 py-1 text-xs font-medium ${attendanceMap[st.id] === 'present' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>{attendanceMap[st.id]}</span>
                  </td>
                  <td className="px-5 py-4">
